...
export default function ListBox({ show, options, onSelect, displayKey = "label" }) {
                ...
                <Match when={options()}>
                  {q => {
                    ...
                    <For each={options()}>
                        {(x, i) => {
                            // GitUrlParse
                            return (<li class="text-gray-900 cursor-default select-none relative py-2 pl-10 pr-4 hover:bg-red-100">
                              <span class={`${selected() === i() ? "font-medium" : "font-normal"} block truncate !hover:text-amber-900 !hover:bg-amber-100`} onClick={() => {
                                    setSelected(i());
                                    onSelect(i());
                                }}>
                                {x[displayKey]}
